...
void rexnet_stream_connect(RexNetHandle* handle, uint32_t virtual_ip,
                           uint16_t src_port, uint16_t dst_port);
/** Write to a guest TCP connection. */
void rexnet_stream_send(RexNetHandle* handle, uint64_t stream_id,
...
uint32_t rexnet_friend_code(const RexNetPeerId* peer, const char* name, char* out,
                            uint32_t cap);
...
void rexnet_invite_reply(RexNetHandle* handle, const RexNetPeerId* peer,
                         bool accept);
...
void rexnet_friend_request(RexNetHandle* handle, const RexNetPeerId* peer,
                           const char* note);
...
void rexnet_session_create(RexNetHandle* handle, const uint8_t* session_id,
                           uint8_t slots_total, uint8_t slots_open,
                           bool is_public);
...
void rexnet_send_datagram(RexNetHandle* handle, uint32_t virtual_ip,
                          uint16_t src_port, uint16_t dst_port,
                          const uint8_t* data, uint32_t len, bool reliable);
